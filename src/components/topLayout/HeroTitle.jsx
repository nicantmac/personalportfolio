...
function HeroTitle() {
    ...
    return (
        <section className="hero-container"> {/* hero entire container */}
            {/* bubble looking container */}
            <Motion.main className="hero-main"
                  variants={container} initial="hidden" animate="show"
            >
                <Header/> {/* renders header */}
                <Motion.h1 className="hero-intro" variants={item}>
                    {t('hero.title')}<span className="wave" role="img" aria-label="wave">👋🏼</span>
                </Motion.h1>

                <Motion.h2 className="hero-name" variants={item}>
                    {t('hero.name')}
                </Motion.h2>
                ...
                <Motion.div className="hero-btns" variants={item}>
                    <button onClick={() => setModal(true)} className="talk-btn" aria-label="bio section">
                        {t('hero-btn.talk')}
                ...
                <Modal isOpen={modalOpen} onClose={() => setModal(false)}></Modal>

                {/* social links */}
                <Motion.div className="social-icons" variants={item}>
                    {/* anchor tags: directs user to (LinkedIn) or (GitHub) */}
                    <a href="https://www.linkedin.com/in/nicholasmack972/"
                       target="_blank" className="icon linkedin" aria-label="LinkedIn">
                    ...
                    <a href="https://github.com/nicantmac?tab=repositories"
                       target="_blank" className="icon github" aria-label="GitHub">
                        <svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
